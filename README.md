...
本仓库的核心目标不是存放普通脚本，而是为 Codex 提供可触发、可复查、可维护的 `SKILL.md` 能力包。当前主要包含五类能力：

- 授权 JavaScript 逆向分析与请求复现辅助。
- HTTP 请求分析、参数分类、失败诊断、浏览器依赖判断和可维护请求复现方案设计。
...
| `request-analyzer-898` | `skills/request-analyzer-898/` | HTTP 请求、curl、fetch、axios、HAR、Python requests、浏览器 Network 请求、失败响应和请求复现方案的结构化分析。 |
...
显示标题：`Request Analyzer 898`

用途：面向 HTTP 请求分析、失败请求诊断、参数依赖判断和可维护请求复现方案设计，帮助 Codex 在写代码前先还原请求结构、识别关键参数、判断是否依赖浏览器，并给出可验证的 Python `httpx` 工程方案。

主要能力：

- 解析 `curl`、Chrome DevTools Copy as cURL、`fetch`、`axios`、Python `requests`、HAR、Header 列表和完整 Request/Response。
- 结构化输出 URL、Method、Headers、Cookies、Query、Body 和 Response。
- 自动分类固定参数、动态参数、会话参数、算法参数和环境参数。
- 按固定顺序诊断失败请求：HTTP 层、业务参数、身份状态、算法参数、环境依赖。
- 通过删除测试、修改测试、固定测试和重新生成测试判断参数是否需要处理，不默认全部逆向。
- 分析请求频率、来源环境、设备环境和业务状态，并给出概率排序。
- 判断请求是否可以脱离浏览器，必要时说明缺少的 JS、cookie、token、WebSocket 状态或浏览器存储。
- 在证据充分时生成 Python 3.12 + `httpx` + `asyncio` + `pytest` 的项目结构建议。
- `SKILL.md` 保留核心契约和路由，详细规则拆分到 `rules/`，报告和回答结构放在 `templates/`。
- 默认要求敏感 cookie、token、Authorization、secret 等只记录字段名、来源、长度、依赖和风险，不写入普通报告或测试样本。

适合这样提问：

```text
分析这个 curl 请求，帮我判断哪些参数必须保留、哪些可以本地生成。
```

```text
浏览器里请求成功，Python requests 复现返回 403，帮我按步骤诊断失败原因。
```

```text
根据这个 HAR 和失败响应，判断接口是否能脱离浏览器，并生成可维护的 httpx 项目模板。
```
...
- 任务关键词尽量明确，例如 `curl`、`HAR`、`fetch`、`axios`、`requests`、`Header`、`Cookie`、`sign`、`token`、`key`、`iv`、`jsVMP`、`project memory`、`项目结构图`、`vibe coding`、`代码规范`、`debug`、`code review`。
...
skills/request-analyzer-898/SKILL.md
skills/request-analyzer-898/agents/openai.yaml
skills/request-analyzer-898/rules/scope-and-evidence.md
skills/request-analyzer-898/rules/input-parsing.md
skills/request-analyzer-898/rules/parameter-analysis.md
skills/request-analyzer-898/rules/failure-diagnosis.md
skills/request-analyzer-898/rules/browser-and-stability.md
skills/request-analyzer-898/rules/project-generation.md
skills/request-analyzer-898/templates/analysis-report.md
skills/request-analyzer-898/templates/response-format.md
